...
class PermissionPolicy:
    ...
    def require_approval(
        self, *tool_names: str, callback: ApprovalCallback
    ) -> PermissionPolicy:
    ...
    def sandbox(
        self, allowed_paths: list[str] | None = None, allow_network: bool = True
    ) -> PermissionPolicy:
    ...
    async def check(self, tool_name: str, params: dict[str, Any]) -> tuple[bool, str]:
        ...
        if tool_name in self._allowed_tools:
            ...
            if tool_name in self._approval_callbacks:
                try:
                    allowed = await self._approval_callbacks[tool_name](
                        tool_name, params
                    )
                    if not allowed:
                        return (
                            False,
                            f"Tool {tool_name} requires approval but was denied",
                        )
                ...
                if file_path:
                    try:
                        ...
                        if not allowed:
                            return (
                                False,
                                f"File path {file_path} is outside allowed sandbox paths",
                            )
                    except Exception as e:
                ...
                if cwd:
                    try:
                        ...
                        if not allowed:
                            return (
                                False,
                                f"Bash working directory {cwd} is outside allowed sandbox paths",
                            )
                    except Exception as e:
