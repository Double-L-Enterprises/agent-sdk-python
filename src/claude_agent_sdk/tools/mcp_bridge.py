...
class MCPServerConfig:
    ...
    type: str  # "stdio" | "sse"
    command: str | None = None  # For stdio - e.g., "npx", "python3"
    args: list[str] | None = None  # For stdio - e.g., ["-m", "mcp_server"]
    url: str | None = None  # For sse - e.g., "http://localhost:8030/sse"
...
class MCPBridge:
        ...
        for server_name, config in self.servers.items():
            if config.type == "stdio":
                if not config.command or not config.args:
                    raise ValueError(
                        f"Stdio server {server_name} requires command and args"
                    )
    ...
    async def discover_tools(self) -> list[dict[str, Any]]:
        ...
        for server_name, config in self.servers.items():
            try:
                ...
                logger.warning(
                    "Failed to discover tools from server %s: %s", server_name, exc
                )
                continue

        return all_tools

    def _convert_mcp_to_openai(
        self, mcp_tools: list[dict[str, Any]], server_name: str
    ) -> list[dict[str, Any]]:
    ...
    async def call_tool(
        ...
        try:
            tool_call_params = {"name": tool_name, "arguments": params}

            if config.type == "stdio":
                response = await self._send_jsonrpc_stdio(
                    server_name, "tools/call", tool_call_params
                )
            elif config.type == "sse":
                response = await self._send_jsonrpc_sse(
                    server_name, "tools/call", tool_call_params
                )
            else:
                raise ValueError(f"Unknown server type: {config.type}")
